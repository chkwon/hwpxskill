...
> **[필수] 매 작업 시 원본 HWPX에서 새로 읽기**
>
> 이 스킬의 모든 워크플로우에서, 매번 작업을 시작할 때 반드시 **원본 `.hwpx` 파일에서 직접 XML을 새로 추출**해야 한다.
> - `/tmp`에 이전에 추출한 XML 파일이 있어도 **재사용 금지**
> - `unpack.py`로 풀어놓은 디렉토리가 있어도 **재사용 금지**
> - 사용자가 한글 오피스에서 `.hwpx`를 직접 수정했을 수 있으므로, 이전 XML은 구버전일 수 있다
> - **`.hwpx` 파일이 유일한 신뢰할 수 있는 원본(single source of truth)이다**

> **[필수] 분량은 텍스트로 맞춘다 — 자간·글꼴 크기·장평 조작으로 우겨넣지 말 것**
>
> 본문이 페이지 한도를 넘는다고 해서 다음 시각 속성을 **절대로 임의 변경해서는 안 된다**:
> - `<hh:spacing hangul="..." latin="..." hanja="..." japanese="..." other="..." symbol="..." user="..."/>` — **자간(글자 간격)**
> - `charPr` 의 `height` 속성 — **글꼴 크기(상대 단위, 100 = 10pt)**
> - `<hh:ratio hangul="..." latin="..." .../>` — **장평(글자 폭 비율)**
> - `charPr` 의 `useFontSpace`, `useKerning`, `symMark` — **자간/커닝 보조 옵션**
> - `paraPr` 의 `<hh:lineSpacing type="..." value="..."/>` — **줄 간격**
> - `paraPr` 의 `<hh:margin>` 좌·우·들여쓰기·내어쓰기 값 — **여백/들여쓰기**
>
> **올바른 대응 우선순위**:
> 1. **본문 텍스트를 압축·요약**해서 분량 자체를 줄인다 (제일 먼저 시도).
> 2. 그래도 부족하면 **사용자에게 분량 한도를 명시적으로 확인**받고 한도 완화/페이지 추가 허용을 받는다.
> 3. 한도가 절대적이고 압축이 한계에 달했다면 **사용자에게 상태를 보고**하고 결정을 위임한다.
>
> **금지 이유**: 자간을 좁히거나 글꼴 크기를 임의로 줄여 분량을 맞추는 시각적 트릭은 (1) 가독성을 떨어뜨리고, (2) 심사자·독자에게 비전문적·기만적으로 보이며, (3) 원본 양식에서 정의한 페이지 디자인을 훼손한다. 분량 한계는 **내용 편집 문제**이지 **서식 조작 문제**가 아니다.
>
> **자가 점검**: 작업 완료 직전, 결과 HWPX의 `header.xml`에서 charPr/paraPr/spacing/ratio/lineSpacing 값을 레퍼런스와 비교해 변경된 항목이 없는지 확인한다. 한 항목이라도 변경되었으면 사용자에게 즉시 보고하고 원복한다.
...
# 0) 이전 추출 파일 제거 (항상 원본에서 새로 추출)
rm -f /tmp/ref_header.xml /tmp/ref_section.xml /tmp/new_section0.xml
...
> **주의**: 매번 원본 `.hwpx` 파일에서 새로 unpack해야 한다. 이전에 풀어놓은 `./unpacked/` 디렉토리를 재사용하면 사용자가 한글에서 수정한 내용이 반영되지 않는다.
> 또한 Rule 21에서 설명하듯 `pack.py`의 재패키징은 파일을 손상시킬 수 있으므로, 가능하면 워크플로우 5(레퍼런스 기반) 또는 ZIP 직접 교체 방식을 사용할 것.

```bash
source "$VENV"

# 0. 이전 unpacked 디렉토리 제거 (항상 원본에서 새로 unpack)
rm -rf ./unpacked/
...
# 5. 정리: 풀어놓은 디렉토리 삭제 (다음 작업 시 재사용 방지)
rm -rf ./unpacked/
```
...
1. **[최우선] 항상 원본 HWPX 파일에서 새로 읽기**: 모든 워크플로우에서 매 작업 시작 시 반드시 원본 `.hwpx` 파일에서 XML을 새로 추출(또는 읽기)해야 한다. `/tmp`나 캐시 디렉토리에 이전에 풀어둔(unzip) XML 파일, 또는 이전 반복(iteration)에서 생성한 `./unpacked/` 디렉토리를 **절대 재사용하지 말 것**. 사용자가 한글 오피스에서 직접 HWPX 파일을 수정했을 수 있으므로, 이전에 추출한 XML은 이미 최신 상태가 아닐 수 있다. **`.hwpx` 파일이 유일한 신뢰할 수 있는 원본이다.**
2. **HWPX만 지원**: `.hwp`(바이너리) 파일은 지원하지 않는다. 사용자가 `.hwp` 파일을 제공하면 **한글 오피스에서 `.hwpx`로 다시 저장**하도록 안내할 것. (파일 → 다른 이름으로 저장 → 파일 형식: HWPX)
3. **secPr 필수**: section0.xml 첫 문단의 첫 run에 반드시 secPr + colPr 포함
4. **mimetype 순서**: HWPX 패키징 시 mimetype은 첫 번째 ZIP 엔트리, ZIP_STORED
5. **네임스페이스 보존**: XML 편집 시 `hp:`, `hs:`, `hh:`, `hc:` 접두사 유지
6. **itemCnt 정합성**: header.xml의 charProperties/paraProperties/borderFills itemCnt가 실제 자식 수와 일치
7. **ID 참조 정합성**: section0.xml의 charPrIDRef/paraPrIDRef가 header.xml 정의와 일치
8. **venv 사용**: 프로젝트의 `.venv/bin/python3` (lxml 패키지 필요)
9. **검증**: 생성 후 반드시 `validate.py`로 무결성 확인
...
14. **examples 폴더 미사용**: 작업 중 `.claude/skills/hwpx/examples/*` 파일은 읽기/참조/복사에 사용하지 말 것
15. **쪽수 동일 필수**: 레퍼런스 기반 작업에서는 최종 결과의 쪽수를 레퍼런스와 동일하게 유지할 것
16. **무단 페이지 증가 금지**: 사용자 명시 요청/승인 없이 쪽수 증가를 유발하는 구조 변경 금지
17. **구조 변경 제한**: 사용자 요청이 없는 한 문단/표의 추가·삭제·분할·병합 금지 (치환 중심 편집)
18. **page_guard 필수 통과**: `validate.py`와 별개로 `page_guard.py`를 반드시 통과해야 완료 처리
19. **linesegarray 무효화 주의 (치명적)**: `<hp:linesegarray>` 내부의 `<hp:lineseg textpos="N" .../>` 은 해당 문단 텍스트의 문자 위치(character offset)를 참조한다. **텍스트 길이가 변경되면 textpos 값이 무효화되어 한글 오피스가 파일을 열지 못한다.** 텍스트를 단축하거나 크게 변경할 때는 반드시 해당 문단의 `<hp:linesegarray>...</hp:linesegarray>` 블록 전체를 삭제할 것 — 한글 오피스가 열 때 자동으로 재생성한다. 텍스트 끝에 짧은 괄호 참조(citation)를 추가하는 정도는 textpos 범위 내에서 허용되지만, 텍스트를 대폭 단축/삭제하면 반드시 linesegarray를 함께 제거해야 한다. 특히 **표(table) 셀 내부 문단**에서 텍스트를 줄일 때 이 문제가 발생하기 쉽다.
20. **lxml 재직렬화 금지**: lxml의 `tree.write()` 또는 `etree.tostring()`으로 XML을 재직렬화하면 (1) `standalone="yes"` 누락, (2) 인용부호 변경(`"` → `'`), (3) 줄바꿈/공백 삽입, (4) 네임스페이스 재배치 등이 발생하여 한글 오피스가 파일을 열지 못할 수 있다. **기존 HWPX를 편집할 때는 원본 XML 바이트를 문자열로 읽어서 `str.replace()`로 치환하고 다시 바이트로 인코딩하는 방식을 사용할 것.** lxml은 분석/파싱 용도로만 사용한다.
21. **ZIP 구조 보존 필수**: `pack.py`로 재패키징하면 파일 순서, 압축 방식(BinData PNG는 ZIP_STORED), external_attr 등이 변경되어 한글 오피스가 열지 못할 수 있다. **기존 HWPX를 편집할 때는 원본 ZIP의 ZipInfo를 그대로 유지하면서 변경된 XML만 `writestr(item, new_data)`로 교체하는 방식을 사용할 것:**
    ```python
    with zipfile.ZipFile(src, 'r') as zin:
        with zipfile.ZipFile(dst, 'w') as zout:
            for item in zin.infolist():
    ...
    ```
22. **편집 전 백업 필수**: HWPX 파일을 수정하기 전에 반드시 원본의 백업 복사본을 생성할 것. 백업 파일명 규칙: `{원본명}_backup_{YYYYMMDD_HHMMSS}.hwpx`. 이를 통해 수정 실패 시 즉시 롤백할 수 있다.
    ```bash
    cp original.hwpx "original_backup_$(date +%Y%m%d_%H%M%S).hwpx"
    ```
23. **[중요] 자간·글꼴 크기·장평·줄간격 조작으로 분량 우겨넣기 금지**: 페이지 한도에 맞추기 위해 시각 속성을 임의로 변경하는 것은 **절대 금지**한다. 분량 한계는 내용 편집 문제이지 서식 조작 문제가 아니다.

    **금지 대상 XML 속성/요소** (header.xml 또는 section0.xml 어디서든):
    - `charPr` 의 `height` 속성 — 글꼴 크기 (예: `height="1000"` → 10pt; 임의로 `"950"`, `"900"` 등으로 줄이지 말 것)
    - `charPr` 자식 `<hh:spacing>` 의 `hangul`, `latin`, `hanja`, `japanese`, `other`, `symbol`, `user` 속성 — 자간 (예: `hangul="0"` 을 `"-5"`, `"-10"` 등 음수로 좁히지 말 것)
    - `charPr` 자식 `<hh:ratio>` 의 `hangul`, `latin` 등 — 장평 (예: `hangul="100"` 을 `"95"`, `"90"` 으로 좁히지 말 것)
    - `charPr` 의 `useFontSpace`, `useKerning`, `symMark` — 자간/커닝 보조 옵션
    - `paraPr` 자식 `<hh:lineSpacing>` 의 `type`, `value` — 줄간격 (예: `value="160"` 을 `"140"`, `"120"` 으로 좁히지 말 것)
    - `paraPr` 자식 `<hh:margin>` 의 `left`, `right`, `indent` — 좌·우 여백, 들여쓰기·내어쓰기

    **올바른 대응 (우선순위)**:
    1. 본문 텍스트를 **압축·요약**해서 분량 자체를 줄인다.
    2. 그래도 부족하면 사용자에게 **분량 한도 완화 또는 페이지 추가 허용**을 명시적으로 확인받는다.
    3. 한도가 절대적이고 압축이 한계에 달했다면, 결과를 강제로 우겨넣지 말고 **사용자에게 상태를 보고**하고 결정을 위임한다.

    **금지 이유**: (1) 가독성 저하, (2) 심사자·독자에게 비전문적·기만적으로 보임, (3) 원본 양식의 페이지 디자인 훼손, (4) 인쇄·PDF 변환 시 줄 깨짐 위험.

    **자가 점검 (작업 완료 직전 필수)**: 결과 HWPX와 레퍼런스 HWPX의 `header.xml` 을 비교하여 위 속성/요소들이 한 글자도 다르지 않은지 확인한다. 한 항목이라도 변경되었으면 사용자에게 보고하고 원복한다. 사용자가 명시적으로 "글꼴 크기를 X로 바꿔라" 등 서식 변경을 요청한 경우만 예외이며, 이때도 분량 맞추기 목적이면 금지 대상이다.
24. **참고문헌 `&` 표기**: 참고문헌(References) 목록에서 저자 이름 사이의 `&`는 반드시 `&`로 표시되어야 한다. XML에서는 `&amp;`로 인코딩하면 렌더링 시 `&`가 된다. `&amp;amp;`(이중 인코딩)이 들어가면 렌더링 시 `&amp;`로 표시되므로 금지한다. 이 규칙은 `cite_numeric.py` 사용 여부와 관계없이, 참고문헌 텍스트를 직접 작성·편집할 때도 항상 적용된다.
