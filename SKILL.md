...
22. **편집 전 백업 필수**: HWPX 파일을 수정하기 전에 반드시 원본의 백업 복사본을 생성할 것. 백업 파일명 규칙: `{원본명}_backup_{YYYYMMDD_HHMMSS}.hwpx`. 이를 통해 수정 실패 시 즉시 롤백할 수 있다.
    ...
    cp original.hwpx "original_backup_$(date +%Y%m%d_%H%M%S).hwpx"
    ```
23. **글꼴 크기·자간 조작 금지**: 쪽수를 맞추거나 줄 수를 줄이기 위해 글꼴 크기(height), 자간(spacing), 장평(ratio) 등을 임의로 변경하지 말 것. 텍스트가 길어지면 내용을 압축·요약하는 방식으로 대응하고, charPr의 height/spacing 속성이나 paraPr의 lineSpacing 값을 원본과 다르게 변경하는 것은 금지한다. 원본 레퍼런스 문서에 정의된 서식을 그대로 유지할 것.
